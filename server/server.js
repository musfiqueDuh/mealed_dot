...
app.post("/feed", (req, res) => {
  ...
  const hashtags = tweetInfo.content.match(/#[\w]+/g).map(tag => tag.toLowerCase());
...
app.get("/search/hashtag/:tag", async (req, res) => {
  try {
    ...
    res.json({ status: 'ok', tweets });
  } catch (error) {
    res.json({ status: 'error', error: 'Search failed' });
  }
});
